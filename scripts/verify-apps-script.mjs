...
['doGet','doPost','configurePlatform','setupPlatform','setOwnerPassword','installPlatformTriggers','login_','authenticate_','batchMonthlyMappings_','batchTargets_','fetchBigQueryAgents_','importRevenueSheet_'].forEach(name=>{if(!new RegExp(`function\\s+${name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\s*\\(`).test(source))throw new Error(`Missing required Apps Script function: ${name}`);});
